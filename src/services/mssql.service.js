...
exports.insert = async () => {
    ...
    for (let index = 1; index <= 600; index++) {
        await mssql.sequelize.query('INSERT INTO numbers (number) VALUES (:number);',
            { replacements: { number: this.generateNumbers() }, type: mssql.sequelize.QueryTypes.INSERT }
        ).then(numbers => {
            arr.push(numbers);
        })
    }
